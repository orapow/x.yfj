...
namespace X.App.Apis.wx.user {
    public class deposit : _wx {
        ...
        protected override XResp Execute() {
            ...
            var co = Wx.Pay.MdOrder(cu.nickname + "_" + cu.id + "充值", "chg_" + depositLog.charge_id + "", (int)(amount * 100) + "", "http://" + cfg.domain + "/wx/notify-2" + depositLog.charge_id + ".html", cu.wx_opid, cfg.wx_appid, cfg.wx_mch_id, cfg.wx_paykey, false);

            CacheHelper.Remove("pay." + cu.id);
            depositLog.result = (String.IsNullOrWhiteSpace(
                co.return_code)?"SUCCESS":co.return_code);
